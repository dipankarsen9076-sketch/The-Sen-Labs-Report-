...
MASTER_REFS = {
    ...
    "hemoglobin": {"name": "Hemoglobin (Hb)", "unit": "g/dL", "ref": "13.0 - 17.0"},
    ...
    "gran": {"name": "Granulocytes Absolute (#)", "unit": "10^3/uL", "ref": "2.0 - 7.0"},
    ...
    "lym": {"name": "Lymphocytes Absolute (#)", "unit": "10^3/uL", "ref": "1.0 - 3.0"},
    ...
    "mid": {"name": "Mid Cells / Monocytes Absolute (#)", "unit": "10^3/uL", "ref": "0.1 - 0.8"},
    "mid absolute": {"name": "Mid Cells / Monocytes Absolute (#)", "unit": "10^3/uL", "ref": "0.1 - 0.8"},
...
if st.session_state.extracted_tests:
    ...
    try:
        hb_val = float(str(final_data.get("HGB", final_data.get("Hemoglobin", 13.5))).replace(",", "."))
    except:
        hb_val = 13.5
    ...
    try:
        plt_val = float(str(final_data.get("PLT", final_data.get("Platelets", 250))).replace(",", "."))
    except:
        plt_val = 250.0

    # Intelligent clinical baseline suggestions
    if hb_val < 12.0 and mcv_val < 80.0:
        def_rbc = "Microcytic, hypochromic red cells with mild anisopoikilocytosis. Few pencil cells seen."
        def_imp = "Microcytic Hypochromic Anemia (Probable Iron Deficiency Anemia). Advised: Serum Ferritin / Iron Profile."
    elif hb_val < 12.0:
        def_rbc = "Normocytic, normochromic to mildly hypochromic red blood cells. No abnormal inclusion bodies."
        def_imp = "Mild Anemia. Advised clinical correlation and follow up."
    else:
        def_rbc = "Predominantly normocytic and normochromic. Central pallor is normal. No nucleated RBCs seen."
        def_imp = "Blood smear examination reveals normal hematological profile. No specific pathology identified."

    if plt_val < 150:
        def_plt = "Reduced on smear. No significant giant platelets or platelet aggregation seen."
    else:
        def_plt = "Adequate in number, seen singly and in small clumps. Normal morphology."

    def_wbc = "Normal in total count and distribution. Mature granulocytes seen without shift to left. No immature or blast cells."
    def_parasite = "No hemoparasites (Malarial Parasite / Microfilaria) seen on smear examination."
    ...
    gbp_imp = st.text_area("Diagnostic Impression / Advice", value=def_imp, height=68)
    ...
    if st.button("Generate Complete CBC + GBP PDF Report"):
        ...
        c.drawString(32, height - 47, "ADVANCED PATHOLOGY & CLINICAL LABORATORY | AUTOMATED HEMATOLOGY & CYTOMORPHOLOGY")
        ...
        c.drawString(38, height - 128, "COMPLETE BLOOD COUNT (AUTOMATED 3-PART HEMATOLOGY PROFILE)")
        ...
        # 5. Compact CBC Table Rows (10.8pt line height to cleanly fit 18 tests in top half)
        y = height - 156
        ...
        # 6. GBP / Peripheral Blood Smear Banner
        gbp_top = y - 4
        c.setFillColor(colors.HexColor("#1e3a8a"))
        c.rect(32, gbp_top, width - 64, 12, fill=True, stroke=False)
        c.setFillColor(colors.white)
        c.setFont("Helvetica-Bold", 7)
        c.drawString(38, gbp_top + 3, "GENERAL BLOOD PICTURE (PERIPHERAL BLOOD SMEAR EXAMINATION)")
