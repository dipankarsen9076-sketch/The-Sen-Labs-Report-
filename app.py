    ...
    with st.spinner("Analyzing machine screen and extracting all parameters..."):
      ...
      for m_name in candidate_models:
        try:
          ...
          st.success(
              f"Extracted {len(st.session_state.extracted_tests)} parameters"
              " successfully!"
          )
          success = True
          break
      ...
      if not success:
        st.error(
            f"Extraction busy/error. Please click again in 5 seconds. ({last_err})"
        )
...
if st.session_state.extracted_tests:
  ...
  for test, val in st.session_state.extracted_tests.items():
    ...
    with cols[idx % 2]:
      final_data[test] = st.text_input(
          f"{clean_label}", value=str(val), key=f"inp_{test}"
      )
    idx += 1
  ...
  if st.button("Generate Complete Professional PDF Report"):
    ...
    c.drawString(
        36,
        height - 52,
        "ADVANCED PATHOLOGY & CLINICAL LABORATORY | 3-PART & 5-PART AUTOMATED"
        " HEMATOLOGY",
    )
    c.drawRightString(
        width - 36, height - 40, f"Helpdesk: +91 {p_contact}"
    )
