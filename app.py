...
if "Complete Blood Count (CBC + GBP)" in selected_profiles:
    ...
    final_report_sections["CBC"] = {
        "items": {
            # Name: (val, method, unit, ref, low_val, high_val)
            "Hemoglobin (Hb)": (hgb, "SLS / Cyanmethemoglobin", "g/dL", "13.0 - 17.0", 13.0, 17.0),
            "Total RBC Count": (rbc, "Electrical Impedance", "10^6/uL", "4.50 - 5.50", 4.50, 5.50),
            "Packed Cell Volume (PCV/HCT)": (hct, "Calculated / Centrifugation", "%", "40.0 - 50.0", 40.0, 50.0),
            "Mean Corpuscular Volume (MCV)": (mcv, "Calculated", "fL", "80.0 - 100.0", 80.0, 100.0),
            "Mean Corpuscular Hemoglobin (MCH)": (mch, "Calculated", "pg", "27.0 - 32.0", 27.0, 32.0),
            "Mean Corpuscular Hb Conc. (MCHC)": (mchc, "Calculated", "g/dL", "31.5 - 35.5", 31.5, 35.5),
            "RDW - SD (RDWa)": (rdwa, "Direct Volume Histogram", "fL", "39.0 - 46.0", 39.0, 46.0),
            "RDW - CV": (rdw_cv, "Calculated", "%", "11.5 - 14.5", 11.5, 14.5),
            "Total Leukocyte Count (WBC/TLC)": (wbc, "Electrical Impedance", "10^3/uL", "4.0 - 10.0", 4.0, 10.0),
            "Granulocytes / Neutrophils (%)": (gran_p, "Electrical Impedance", "%", "40.0 - 70.0", 40.0, 70.0),
            "Lymphocytes (%)": (lym_p, "Electrical Impedance", "%", "20.0 - 40.0", 20.0, 40.0),
            "Mid Cells / Monocytes (%)": (mid_p, "Electrical Impedance", "%", "2.0 - 8.0", 2.0, 8.0),
...
if "Liver Function Test (LFT)" in selected_profiles:
    ...
    final_report_sections["LFT"] = {
        "Serum Bilirubin Total": (b_tot, "Diazo Method (Malloy-Evelyn)", "mg/dL", "0.2 - 1.2", 0.2, 1.2),
        "Serum Bilirubin Direct": (b_dir, "Diazo Coupling Reaction", "mg/dL", "0.0 - 0.3", 0.0, 0.3),
        "Serum Bilirubin Indirect": (b_ind, "Calculated (Total - Direct)", "mg/dL", "0.2 - 0.9", 0.2, 0.9),
        "SGOT / AST": (sgot, "IFCC without Pyridoxal Phosphate", "U/L", "5 - 40", 5.0, 40.0),
        "SGPT / ALT": (sgpt, "IFCC without Pyridoxal Phosphate", "U/L", "5 - 45", 5.0, 45.0),
        "AST : ALT (De Ritis) Ratio": (de_ritis, "Calculated", "Ratio", "0.8 - 1.5", 0.8, 1.5),
        "Alkaline Phosphatase (ALP)": (alp, "p-NPP Kinetic Method", "U/L", "80 - 306", 80.0, 306.0),
        "Serum Total Protein": (t_prot, "Biuret Method", "g/dL", "6.4 - 8.3", 6.4, 8.3),
        "Serum Albumin": (alb, "BCG (Bromocresol Green)", "g/dL", "3.5 - 5.2", 3.5, 5.2),
        "Serum Globulin": (glob, "Calculated (Protein - Albumin)", "g/dL", "2.0 - 3.5", 2.0, 3.5),
...
if "Kidney Function Test (KFT / RFT)" in selected_profiles:
    ...
    final_report_sections["KFT"] = {
        ...
        "Serum Creatinine": (creat, "Modified Jaffe's Kinetic", "mg/dL", "0.6 - 1.4", 0.6, 1.4),
        "Blood Urea Nitrogen (BUN)": (bun, "Calculated (Urea / 2.14)", "mg/dL", "7.0 - 20.0", 7.0, 20.0),
        "Urea / Creatinine Ratio": (u_cr_ratio, "Calculated", "Ratio", "20.0 - 35.0", 20.0, 35.0),
        "Serum Uric Acid": (uric, "Uricase / POD Method", "mg/dL", "3.5 - 7.2", 3.5, 7.2),
        "Serum Calcium": (calcium, "Arsenazo III Method", "mg/dL", "8.5 - 10.5", 8.5, 10.5),
        "Serum Sodium (Na+)": (sod, "Ion Selective Electrode (ISE)", "mEq/L", "135 - 145", 135.0, 145.0),
        "Serum Potassium (K+)": (pot, "Ion Selective Electrode (ISE)", "mEq/L", "3.5 - 5.0", 3.5, 5.0),
        "Serum Chloride (Cl-)": (chlor, "Ion Selective Electrode (ISE)", "mEq/L", "96 - 106", 96.0, 106.0)
    }
...
if "Lipid Profile" in selected_profiles:
    ...
    final_report_sections["LIPID"] = {
        "Total Cholesterol": (chol, "CHOD - PAP Method", "mg/dL", "< 200 (Desirable)", 0.0, 200.0),
        "Serum Triglycerides": (trig, "GPO - PAP Method", "mg/dL", "< 150 (Normal)", 0.0, 150.0),
        "HDL Cholesterol (Good)": (hdl, "Direct Enzymatic Clearance", "mg/dL", "> 40 (Optimal)", 40.0, 999.0),
        "LDL Cholesterol (Bad)": (ldl, "Calculated (Friedewald Formula)", "mg/dL", "< 100 (Optimal)", 0.0, 100.0),
        "VLDL Cholesterol": (vldl, "Calculated (TG / 5)", "mg/dL", "5.0 - 30.0", 5.0, 30.0),
        "Non-HDL Cholesterol": (non_hdl, "Calculated (TC - HDL)", "mg/dL", "< 130", 0.0, 130.0),
...
if "Widal Agglutination Test" in selected_profiles:
    ...
    serology_dict["Salmonella typhi 'O'"] = (wo, "Slide Agglutination", "Titer", "Significant >= 1:80", None, None)
    serology_dict["Salmonella typhi 'H'"] = (wh, "Slide Agglutination", "Titer", "Significant >= 1:160", None, None)
    serology_dict["Salmonella paratyphi 'AH'"] = (wah, "Slide Agglutination", "Titer", "Negative / Non-significant", None, None)
    serology_dict["Salmonella paratyphi 'BH'"] = (wbh, "Slide Agglutination", "Titer", "Negative / Non-significant", None, None)
...
if "Typhidot (IgM / IgG)" in selected_profiles:
    ...
    serology_dict["Typhidot IgM Antibody"] = (ty_m, "Immunochromatography", "Qualitative", "Non-Reactive (Negative)", None, None)
    serology_dict["Typhidot IgG Antibody"] = (ty_g, "Immunochromatography", "Qualitative", "Non-Reactive (Negative)", None, None)
...
if "Malaria Card & Smear (MP)" in selected_profiles:
    ...
    serology_dict["Malarial Parasite (Smear)"] = (mp_smear, "Giemsa / Leishman Microscopy", "Microscopy", "No Parasites Seen", None, None)
    serology_dict["P. vivax Antigen"] = (mp_pv, "Immunochromatography (HRP-2/pLDH)", "Rapid Card", "Negative", None, None)
    serology_dict["P. falciparum Antigen"] = (mp_pf, "Immunochromatography (HRP-2/pLDH)", "Rapid Card", "Negative", None, None)
...
if "Blood Glucose" in selected_profiles:
    ...
    if glu_f: serology_dict["Blood Glucose (Fasting)"] = (glu_f, "GOD - POD Method", "mg/dL", "70.0 - 100.0", 70.0, 100.0)
    if glu_pp: serology_dict["Blood Glucose (PP)"] = (glu_pp, "GOD - POD Method", "mg/dL", "70.0 - 140.0", 70.0, 140.0)
    if glu_r: serology_dict["Blood Glucose (Random)"] = (glu_r, "GOD - POD Method", "mg/dL", "70.0 - 140.0", 70.0, 140.0)

if serology_dict:
    final_report_sections["SEROLOGY"] = serology_dict

# ----------------- CLINICAL INTERPRETATION / GUIDANCE ADVICE GENERATOR -----------------
def generate_profile_summary(section_key, active_items):
    ...
    for name, data in active_items.items():
        ...
        elif "Positive" in val_str or "Reactive" in val_str or "seen" in val_str.lower():
            abnormal_flags.append(f"{name} is REACTIVE / POSITIVE")

    if not abnormal_flags:
        return "Clinical Note: Evaluated parameters are within biological reference intervals. No immediate intervention required."

    if section_key == "CBC":
        notes.append("Clinical Impression: Mild variation in hemogram observed.")
        if any("Hemoglobin" in x and "Decreased" in x for x in abnormal_flags):
            notes.append("Advice: Hemoglobin is low indicating Anemia. Recommend Iron studies (Serum Ferritin) and dietary hematinics.")
        if any("Platelet" in x and "Decreased" in x for x in abnormal_flags):
            notes.append("Advice: Thrombocytopenia noted. Advise monitoring platelet count and clinical screening for viral etiology / dengue.")
        if any("WBC" in x and "Elevated" in x for x in abnormal_flags):
            notes.append("Advice: Leukocytosis suggests acute inflammatory or bacterial response. Correlate clinically.")

    elif section_key == "LFT":
        notes.append("Clinical Impression: Hepatic parameter elevation noted.")
        if any("Bilirubin" in x and "Elevated" in x for x in abnormal_flags):
            notes.append("Advice: Hyperbilirubinemia noted. Suggest abdominal ultrasound (USG Hepatobiliary) and viral hepatitis screening.")
        if any(("SGOT" in x or "SGPT" in x) and "Elevated" in x for x in abnormal_flags):
            notes.append("Advice: Transaminase elevation indicates ongoing hepatocellular injury. Avoid hepatotoxic drugs and correlate clinically.")

    elif section_key == "KFT":
        notes.append("Clinical Impression: Renal profile variation noted.")
        if any(("Urea" in x or "Creatinine" in x) and "Elevated" in x for x in abnormal_flags):
            notes.append("Advice: Elevated Urea/Creatinine points toward reduced renal clearance or pre-renal dehydration. Check fluid intake and follow up.")
        if any("Uric Acid" in x and "Elevated" in x for x in abnormal_flags):
            notes.append("Advice: Hyperuricemia noted. Advise low-purine diet and hydration.")

    elif section_key == "LIPID":
        notes.append("Clinical Impression: Dyslipidemia pattern observed.")
        if any(("Cholesterol" in x or "Triglycerides" in x or "LDL" in x) and "Elevated" in x for x in abnormal_flags):
            notes.append("Advice: Elevated atherogenic lipids. Advise lifestyle modification, low-fat diet, regular physical exercise and physician consult.")

    elif section_key == "SEROLOGY":
        if any("Widal" in x or "typhi" in x for x in abnormal_flags):
            notes.append("Advice: Significant agglutination titers noted for Enteric (Typhoid) fever. Correlate with duration of fever and blood culture.")
        if any("Malaria" in x and "seen" in x.lower() for x in abnormal_flags):
            notes.append("Advice: Malarial parasite demonstrated. Initiate immediate antimalarial therapy under medical supervision.")

    return " | ".join(notes) if notes else "Clinical Correlation: " + ", ".join(abnormal_flags[:3]) + ". Advised physician consultation."
...
class PDFReportManager:
    ...
    def print_section(self, section_key, title, data_dict, bar_hex):
        ...
        # Table Header (Added METHOD column)
        ...
        self.c.drawString(205, self.curr_y - 22, "TEST METHOD")
        self.c.drawString(310, self.curr_y - 22, "RESULT")
        self.c.drawString(370, self.curr_y - 22, "UNIT")
        self.c.drawString(435, self.curr_y - 22, "REFERENCE INTERVAL")
        ...
        for param, (val, method, unit, ref, low_val, high_val) in active_rows.items():
            ...
            self.c.setFillColor(colors.HexColor("#64748b"))
            self.c.drawString(205, y, str(method)[:24])

            # Dynamic Bold Highlight for High / Low
            if is_abnormal:
                self.c.setFont("Helvetica-Bold", 7.0)
                self.c.setFillColor(colors.HexColor("#b91c1c"))  # Professional Medical Crimson Red
                self.c.drawString(310, y, f"{val_str}{flag_suffix}")
            else:
                self.c.setFont("Helvetica", 6.8)
                self.c.setFillColor(colors.HexColor("#0f172a"))
                self.c.drawString(310, y, str(val_str))

            self.c.setFont("Helvetica", 6.8)
            self.c.setFillColor(colors.HexColor("#0f172a"))
            self.c.drawString(370, y, str(unit))
            self.c.drawString(435, y, str(ref))

            self.c.setStrokeColor(colors.HexColor("#f1f5f9"))
            self.c.line(32, y - 2, self.width - 32, y - 2)
            y -= 10.8

        # Profile Interpretation / Guidance Advice Box
...
if not selected_profiles:
    ...
    if st.button("🖨️ Generate Professional Pathology Report (PDF)"):
        ...
        if "CBC" in final_report_sections:
            doc.print_section("CBC", "COMPLETE BLOOD COUNT (AUTOMATED HEMATOLOGY PROFILE)", final_report_sections["CBC"]["items"], "#0f766e")
            doc.print_gbp_box(final_report_sections["CBC"]["gbp"])
            
        # 2. LFT
        if "LFT" in final_report_sections:
            doc.print_section("LFT", "CLINICAL BIOCHEMISTRY - LIVER FUNCTION TEST (LFT)", final_report_sections["LFT"], "#854d0e")
            
        # 3. KFT
        if "KFT" in final_report_sections:
            doc.print_section("KFT", "CLINICAL BIOCHEMISTRY - KIDNEY FUNCTION TEST (KFT)", final_report_sections["KFT"], "#7c2d12")
            
        # 4. Lipid
        if "LIPID" in final_report_sections:
            doc.print_section("LIPID", "CLINICAL BIOCHEMISTRY - LIPID PROFILE", final_report_sections["LIPID"], "#be123c")
            
        # 5. Serology
        if "SEROLOGY" in final_report_sections:
            doc.print_section("SEROLOGY", "SEROLOGY & RAPID CLINICAL IMMUNOLOGY", final_report_sections["SEROLOGY"], "#4338ca")
